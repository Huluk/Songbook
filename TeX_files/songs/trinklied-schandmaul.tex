\songtitle{Trinklied}{Schandmaul}{1999}

\guitarchord{D}
\guitarchord{A}
\guitarchord{Bm}
\guitarchord{G}
\guitarchord{Fsharpm}

\begin{guitar}
	\songsection{Strophe 1}
	[D]Losgelöst und [A]ohne Sorgen [Bm]kein Ge[G]danke [D]an den [A]Morgen,
	[D]Woll'n wir heut' zu[A]sammensein [Bm]und an [G]Wein und [A]Bier und er[D]freu'n.\vspace{0.1em}
	\adjustboxDefault{Der [D]Tisch soll reich ge[A]deckt heut sein, am [Bm]Spieß da [G]schmort ein [D]ganzes [A]Schwein.}
	[D]Und wir halten [A]hoch die Krüge, des [Bm]Trinkens [G]werden [A]wir nicht [D]müde.
	
	\songsection{Bridge}
	Zur [G]Freude soll Mu[F#m]sik erklingen, [G]wer noch kann soll [D]dazu [A]singen
	[G] Und, wenn nicht zu [F#m]voll der Ranzen,[G] fröhlich auf den [A]Tischen tanzen.
	
	\begin{highlightbar}
		\songsection{Refrain}
		Es [D]fließt der Wein, es [A]fließt das Bier. [G]Hoch die Krüge, [A]trinken wir!
		Es [D]fließt der Wein, es [A]fließt das Bier. [G]Hoch die Krüge, [A]trinken wir!
	\end{highlightbar}
	
	\songsection{Instrumental} {\footnotesize\begin{tabular}{|l|l|l|l||l|l|l|l|}
			D & A & Bm G & D A & D & A & Bm G & A D
	\end{tabular}}
	
	\songsection{Strophe 2}
	Auf dem Schoß ein schönes Weib und du berührst den zarten Leib.
	Schaust sie an mit tiefem Blick und willst nie mehr nach Haus zurück.
	Neben dir ein schöner Mann, nimmt dich sachte bei der Hand.
	Preist dich deines Anblicks$^1$ wegen, will dir die Welt zu Füßen legen.
	
	\songsection{Bridge} \songsnippet{Zur Freude soll Musik erklingen, wer noch kann soll dazu singen...}
	
	\begin{highlightbar}
		\songsection{Refrain} \songsnippet{Es fließt der Wein, es fließt das Bier. Hoch die Krüge, trinken wir!}
	\end{highlightbar}
	
	\songsection{Instrumental} {\footnotesize\begin{tabular}{|l|l|l|l||l|l|l|l|}
			D & A & Bm G & D A & D & A & Bm G & A D
	\end{tabular}}
	
	\songsection{Strophe 3}
	Erzählt 'nen Schwank aus eurem Leben, denn was könnt' es schön'res geben,
	Als bei Grölen und bei Lachen, die lange Nacht zum Tag zu machen.
	So wollen wir die Nacht verbringen mit schönen, angenehmen Dingen.
	Trübsal wollen wir vertreiben, wir lassen den Spaß uns nicht verleiden.
	
	\songsection{Bridge} \songsnippet{Zur Freude soll Musik erklingen, wer noch kann soll dazu singen...}
	
	\begin{highlightbar}
		\songsection{Refrain} \songsnippet{Es fließt der Wein, es fließt das Bier. Hoch die Krüge, trinken...} \optionalChord{(x2)}
	\end{highlightbar}
	
	\songsection{Instrumental} {\footnotesize\begin{tabular}{|l|l|l|l||l|l|l|l|}
			D & A & Bm G & D A & D & A & Bm G & A D
	\end{tabular}} \optionalChord{(x2)}
\end{guitar}
\vfill
{\footnotesize \optionalChord{$^1$ Textänderung: (\textit{orig.}) "deiner Schönheit wegen". "Schön" wiederholt sich zu oft.}}




